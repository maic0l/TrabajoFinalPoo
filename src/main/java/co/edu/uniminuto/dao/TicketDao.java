...
public class TicketDao {
    ...
    public Ticket registrarTicket(Ticket ticket) {
        ...
        try {
            ...
            if (ticket.getVehiculo() instanceof Carro) {
                ...
                this.statement.setString(7, "No aplica placa"); // Caso para otro tipo de vehículo
            }
            
            // Establecimiento de la fecha de entrada y salida del vehículo en el ticket
            ...
            // Establecimiento del pago y número de parqueadero en el ticket
            ...
            // Mensaje de éxito si se insertó correctamente el ticket
            if (respuesta > 0) {
    ...
    public void actualizarFechaSalida(LocalDateTime fechaGeneradaUsuario, String nombre, int idUsuario, int ticketNum) {
        ...
        PreparedStatement selectStatement = null; // Declaración del PreparedStatement para la consulta
        PreparedStatement updateStatement = null; // Declaración del PreparedStatement para la actualización
        ...
        try {
            ...
            selectStatement.setString(1, nombre); // Establecimiento del parámetro de nombre de usuario
            selectStatement.setInt(2, idUsuario); // Establecimiento del parámetro de ID de usuario
            selectStatement.setInt(3, ticketNum); // Establecimiento del parámetro de número de ticket
            ...
            updateStatement.setString(1, fechaGeneradaUsuario.format(formatter)); // Establecimiento del parámetro de fecha de salida
            updateStatement.setDouble(2, pago); // Establecimiento del parámetro de pago
            updateStatement.setString(3, nombre); // Establecimiento del parámetro de nombre de usuario
            updateStatement.setInt(4, idUsuario); // Establecimiento del parámetro de ID de usuario
            updateStatement.setInt(5, ticketNum); // Establecimiento del parámetro de número de ticket
    ...
    public void borrarTicket(String nombre, int idUsuario, int numeroDeTicket){
        Connection con = conexion.getconectar(); // Obtención de la conexión
        String query = "DELETE FROM ticket WHERE tick_usuario_nombre = ? AND tick_usuario_id = ? AND tick_id = ?"; // Query SQL para el borrado
    ...
    public void consultarTicket(int numeroDeTicket){
        ...
        try {
            ...
            if (resultSet.next()) {                
                ...
                // Mostrar un mensaje si no se encontró el número de ticket
    ...
    public void borrarStatus(LocalDateTime fechaGeneradaUsuario, String nombre, int idUsuario, int ticketNum) {
        ...
        try {
            ...
            // Query SQL para actualizar el status y el pago del ticket
            String updateQuery = "UPDATE ticket SET tick_status = 0, tick_vehiculo_salida = ?, tick_pago = ? WHERE tick_usuario_nombre = ? AND tick_usuario_id = ? AND tick_id = ?";
            updateStatement = con.prepareStatement(updateQuery);
            updateStatement.setString(1, fechaGeneradaUsuario.format(formatter)); // Establecimiento del parámetro de fecha de salida
            updateStatement.setDouble(2, pago); // Establecimiento del parámetro de pago
            updateStatement.setString(3, nombre); // Establecimiento del parámetro de nombre de usuario
